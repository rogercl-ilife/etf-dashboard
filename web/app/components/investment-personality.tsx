...
> = {
  en: {
    ...
    labels: {
      ...
      algoReturn:
        'Estimated annual return = weighted annualized return from ETF snapshots (10Y fallback to 5Y/3Y/1Y).',
  ...
  'zh-TW': {
    ...
    labels: {
      ...
      trustTitle: '試算依據',
      trustHorizon: '期間',
      trustHorizonValue: '10 年',
      trustFormula: '公式',
      trustFormulaValue: 'Base = 加權年化報酬；Bull = Base + 2%；Bear = Base - 3%',
      trustSource: '報酬來源',
      trustSourceValue: '`etf_snapshots`：10Y 缺值回退至 5Y/3Y/1Y',
      trustAllocation: '本次ETF權重',
      trustCoverage: '資料覆蓋率',
      trustSummaryTitle: '摘要',
      trustSummaryFormula: '10年期間 / Base、Bull=Base+2%、Bear=Base-3%',
      algoTitle: '算法說明',
      algoHow: '先套用人格的三類資產權重，再把每類權重平均分配到該類 ETF 清單。',
      algoReturn: '估計年化報酬 = ETF 快照報酬加權年化（優先 10Y，缺值改 5Y/3Y/1Y）。',
  ...
  'zh-CN': {
    ...
    labels: {
      ...
      trustCoverage: '数据覆盖率',
      trustSummaryTitle: '摘要',
      trustSummaryFormula: '10年期间 / Base、Bull=Base+2%、Bear=Base-3%',
      algoTitle: '算法说明',
      algoHow: '先套用人格的三类资产权重，再把每类权重平均分配到该类 ETF 清单。',
      algoReturn: '估计年化回报 = ETF 快照回报加权年化（优先 10Y，缺值改 5Y/3Y/1Y）。',
